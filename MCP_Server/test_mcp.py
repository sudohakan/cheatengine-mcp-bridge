...
def has_field(field: str, field_type: type = None):
    ...
    def validator(resp):
        ...
        if field_type and not isinstance(resp[field], field_type):
            return (
                False,
                f"Field '{field}' should be {field_type.__name__}, got {type(resp[field]).__name__}",
            )
        return True, ""
...
def field_in_range(field: str, min_val, max_val):
    ...
    def validator(resp):
        ...
        if not (min_val <= val <= max_val):
            return (
                False,
                f"Field '{field}' = {val}, expected range [{min_val}, {max_val}]",
            )
        return True, ""
...
def array_min_length(field: str, min_len: int):
    ...
    def validator(resp):
        ...
        if len(resp[field]) < min_len:
            return (
                False,
                f"Field '{field}' has {len(resp[field])} items, expected >= {min_len}",
            )
        return True, ""
...
def mz_header_check():
    ...
    def validator(resp):
        ...
        if bytes_arr[0] != 0x4D or bytes_arr[1] != 0x5A:
            return (
                False,
                f"Expected MZ header (4D 5A), got {bytes_arr[0]:02X} {bytes_arr[1]:02X}",
            )
        return True, ""
...
def version_check(expected_prefix: str):
    ...
    def validator(resp):
        ...
        if not resp["version"].startswith(expected_prefix):
            return (
                False,
                f"Version '{resp['version']}' doesn't start with '{expected_prefix}'",
            )
        return True, ""
...
def main():
    ...
    for test in [
        ...
        "get_process_info",
        "evaluate_lua_simple",
        "evaluate_lua_complex",
        "evaluate_lua_targetIs64Bit",
    ]:
        all_tests[test].run(client)

    # Get arch info for later tests
    arch_result = client.send_command(
        "evaluate_lua", {"code": "return tostring(targetIs64Bit())"}
    )
    ...
    all_tests["read_memory"] = TestCase(
        "Read Memory (16 bytes from PE header)",
        "read_memory",
    ...
    all_tests["read_integer_byte"] = TestCase(
        "Read Integer (byte) - should be 0x4D (M)",
        "read_integer",
    ...
    all_tests["read_integer_word"] = TestCase(
        "Read Integer (word) - should be 0x5A4D (ZM little-endian)",
        "read_integer",
    ...
    all_tests["read_string"] = TestCase(
        ...
        validators=[
            ...
            lambda r: (
                "MZ" in r.get("value", "") or r["value"].startswith("MZ"),
    ...
    entry_point = None
    pe_offset_result = client.send_command(
        "read_integer", {"address": module_base + 0x3C, "type": "dword"}
    )
    if pe_offset_result.get("result", {}).get("success"):
        pe_offset = pe_offset_result["result"]["value"]
        entry_rva_result = client.send_command(
            "read_integer", {"address": module_base + pe_offset + 0x28, "type": "dword"}
        )
    ...
    all_tests["disassemble"] = TestCase(
        "Disassemble (5 instructions from entry point)",
        "disassemble",
        ...
        validators=[
            ...
            lambda r: (
                all(
                    "address" in i and "bytes" in i and "instruction" in i
    ...
    all_tests["enum_memory_regions_full"] = TestCase(
        "Enum Memory Regions Full (Native API)",
    ...
    all_tests["get_physical_address"] = TestCase(
        ...
        validators=[
            ...
            lambda r: (
                not r.get("success") or "physical_address" in r,
    ...
    dbvm_available = all_tests[
        "get_physical_address"
    ].result == TestResult.PASSED and all_tests["get_physical_address"].response.get(
        "success"
    )
    ...
    if dbvm_available:
        ...
        all_tests["start_dbvm_watch"] = TestCase(
            ...
            validators=[
                ...
                lambda r: (
                    not r.get("success") or "watch_id" in r,
                    "Missing watch_id on success",
                ),
                lambda r: (
                    not r.get("success") or r.get("status") == "monitoring",
    ...
    for cat_name, tests in categories.items():
        cat_passed = sum(
            1
            for t in tests
            if all_tests.get(t) and all_tests[t].result == TestResult.PASSED
        )
        cat_failed = sum(
            1
            for t in tests
            if all_tests.get(t) and all_tests[t].result == TestResult.FAILED
        )
        cat_skipped = sum(
            1
            for t in tests
            if all_tests.get(t) and all_tests[t].result == TestResult.SKIPPED
        )
        cat_total = len(tests)
        print(
            f"\n{cat_name}: {cat_passed}/{cat_total - cat_skipped} passed"
            + (f" ({cat_skipped} skipped)" if cat_skipped else "")
        )
        for test in tests:
